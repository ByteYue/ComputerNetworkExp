...
TCPSender::TCPSender():nextseqnum(1), waitingState(false),base(1) {
    this->packges = new vector<Packet*>;
    this->ackCount = new map<int,int>;
}
TCPSender::~TCPSender(){
    if(this->packges!= nullptr){
        delete this->packges;
        this->packges = nullptr;
    }
    if(this->ackCount!= nullptr){
        delete this->packges;
        this->packges = nullptr;
    }
}
...
void TCPSender::printWindow() const {
    cout << "> '-' 表示该包已被确认<" << endl;
    if (base < nextseqnum) {
        cout << "[ ";
